...
export default class EventHighlighter {
  ...
  // TODO: Collect eventIds that are freed - and use them if available
  // TODO use ranges instead of lineNumbe 
  cleanUpMarkers(range) {
    const rowStart = range.start.row;
    const rowEnd = range.end.row;
    ...
    for (const [eventId, cols] of eventIds) {
      for (const [col, marker] of cols) {
        if (
          marker.getBufferRange().start.row >= rowStart
          && marker.getBufferRange().end.row <= rowEnd
        ) {
          cols.delete(col);
        }
      }
      ...
      if (cols.size === 0) {
        ...
        this.unusedEventIds.add(eventId);
      }
    }
  }
  ...
  #updateEventId() {
    ...
    if (this.unusedEventIds.size > 0) {
      const unsedEventIds = [...this.unusedEventIds];
      this.lastEventId = unsedEventIds.pop();

      const reducedUnusedEventIds = [...unsedEventIds];
      this.unusedEventIds = new Set(reducedUnusedEventIds);
    } else {
      this.lastEventId++;
    }
  }
