...
import EventHighlighter from "./event-highlighting/event-highlighter";
...
export default {
    ...
    activate() {
      ...
      if (atom.config.get('tidalcycles.eventHighlighting.enable')) {
        ...
        oscEventHighligthingServer.register("/editor/highlights", this.eventHighlighter.oscHighlightSubscriber());
      ...
      this.remoteControl = new RemoteControl(this.consoleView);
      this.remoteControl.init();

      const oscRemotecontrolServer = new OscServer(this.consoleView, atom.config.get('tidalcycles.remoteControl.ip'), atom.config.get('tidalcycles.remoteControl.port'));
      oscRemotecontrolServer.register("/pulsar/buttons", this.remoteControl.oscPulsarButtons());
      oscRemotecontrolServer.register("/pulsar/button", this.remoteControl.oscPulsarButton());
      oscRemotecontrolServer.register("/pulsar/headlines", this.remoteControl.oscPulsarHeadlines());
      oscRemotecontrolServer.register("/pulsar/audio", this.remoteControl.oscPulsarAudio());
      oscRemotecontrolServer.register("/pulsar/gradient", this.remoteControl.oscPulsarGradient());
      oscRemotecontrolServer.register("/pulsar/event-highlighting/add-style", this.eventHighlighter.handleStyleMessage());
      this.oscServers.push(oscRemotecontrolServer);
