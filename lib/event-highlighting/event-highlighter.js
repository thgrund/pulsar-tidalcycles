...
export default class EventHighlighter {
  ...
  startWorker() {
    ...
    this.worker.onmessage = function(event) {
      const {added, removed, type} = event.data;
      added.forEach((evt) => {
      ...
      if (type === EventHighlightWorkerEvents.RESET) {
  ...
  handleHush(line) {
    if (line.includes("hush") && this.worker) {
        this.worker.postMessage({type: EventHighlightEvents.RESET});
    }
  }
  ...
  oscHighlightSubscriber() {
      return (args: {}): void => {
        ...
        if (this.worker) {
          message.type = EventHighlightEvents.OSC_MESSAGE;
