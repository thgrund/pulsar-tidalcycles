...
export default class EventHighlighter {
  ...
  animate(now) {
    ...
    if (elapsed >= fpsInterval) {
      ...
      [...this.messageBuffer.entries()]
        ...
        .forEach(([key, value]) => {
            activeThisFrame = activeThisFrame.union(value);
      ...
      const { active, added, removed } = EventMapHelper.diffEventMaps(
        this.activeMessages,
        EventMapHelper.transformedEvents(activeThisFrame)
      );
      ...
      this.activeMessages = EventMapHelper.transformedEvents(
  ...
  handleHush(line) {
    if (line.includes("hush") && this.worker) {
        const { removed } = EventMapHelper.diffEventMaps(this.activeMessages, new Map());
