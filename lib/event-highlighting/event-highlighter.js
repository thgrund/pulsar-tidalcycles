...
export default class EventHighlighter {
  ...
  constructor(consoleView, editors) {
    ...
    this.receivedThisFrame = new Map();
  ...
  animate(now) {
    ...
    if (elapsed >= fpsInterval) {
      ...
      removed.forEach(evt => {
        ...
        if (evt.missCount < missThreshold) {
          finalActive.add(evt);
        } else {
          finalRemoved.add(evt);
          // Remove from buffer
          const cols = this.messageBuffer.get(evt.eventId);
          if (cols) {
            cols.delete(evt.colStart);
            if (cols.size === 0) {
              this.messageBuffer.delete(evt.eventId);
            }
          }
        }
      });

      this.messageBuffer = EventMapHelper.transformedEvents(
        [...finalActive, ...finalAdded]
      );

      finalAdded.forEach((evt) => {
        this.#addHighlight(evt);
      });

      finalRemoved.forEach((evt) => this.#removeHighlight(evt));
  ...
  handleHush(line) {
    if (line.includes("hush") && this.worker) {
        this.receivedThisFrame.clear();
        const { removed } = EventMapHelper.diffEventMaps(this.messageBuffer, this.receivedThisFrame);
  ...
  #queueEvent(event) {
    const recvMap = this.#ensureNestedMap(this.receivedThisFrame, event.eventId);
