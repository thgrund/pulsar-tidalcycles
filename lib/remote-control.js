...
export default class RemoteControl {
  ...
  init() {
    ...
    this.server = new OscServer(this.consoleView, 3334);
    this.server.start();
    ...
    this.server.sock.on('message', (msg) => {
      ...
      if (resultMap.address === "/pulsar/buttons") {
        this.buttons.forEach((item, i) => {
          item.className = `flex-item${resultDict["enabled"] < i + 1 ? ' flex-item-disabled' : ''}`
          item.style.cssText = `border: 5px solid ${resultDict["color"]} !important;`;

        });

        this.buttonBorderColor = resultDict["color"];
      }

      if (resultMap.address === "/pulsar/button") {
        const boxShadowStyle = `box-shadow: 0 0 7px 4px ${this.buttonBorderColor} !important;`;

        if (this.activeButton != null && this.activeButton != this.buttons[resultDict["index"]]) {
          this.activeButton.style.cssText = `border: 5px solid ${this.buttonBorderColor} !important;`;
        }

        this.buttons[resultDict["index"]].style.cssText = `border: 5px solid ${this.buttonBorderColor} !important; ${boxShadowStyle}`;
        this.activeButton = this.buttons[resultDict["index"]];
      }

      if (resultMap.address === "/pulsar/headlines") {
          this.songTitleHeadline.innerText = resultDict["songTitle"];
      }

      if (resultMap.address === "/pulsar/audio") {
          this.changeAudioDevice(resultDict["device"]);
      }

      if (resultMap.address === "/pulsar/gradient") {
          this.audioMotion.gradient = resultDict["gradient"];
      }

      console.log("OSC Server Message received", resultMap);
    });
