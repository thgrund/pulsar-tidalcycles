...
export default class REPL {
  ...
  tidalSendExpression(expression, range) {
    ...
    if (this.eventHighlighter && range) {
      this.eventHighlighter.cleanUpMarkers(range);
    }

    // TODO use range for eventHighlighter clean up

    expression.split('\n')
      .forEach((line, index) => {
        let command = line;
        const lineNumber = range ? range.start.row + index : undefined;

        if (this.eventHighlighter) {
          this.eventHighlighter.handleHush(command);
